...
{
    public class CheckClaimGenerationOverride : GatingPluginBase
    {
        public CheckClaimGenerationOverride() : base("ipg_IPGGatingCheckClaimGenerationOverride") { }
        public override GatingResponse Validate(GateManager gateManager, LocalPluginContext ctx = null)
        {
            ...
            {
                return new GatingResponse(false, "Can't determine case record");
            } 
            var claims = GetClaims(ctx);
            var isCaseLocked = IsCaseLocked(ctx);
            var hasClaimGenerationOverrides = HasClaimGenerationOverride(ctx);
            var isGateFailed = claims.Any() && !isCaseLocked && !hasClaimGenerationOverrides;
            var hasClaimGenerationOverridesNoClaim = HasClaimGenerationOverrideNoClaim(ctx);
            ReopenTasks(ctx, ctx.TargetRef(), hasClaimGenerationOverridesNoClaim);
            return new GatingResponse(!isGateFailed);
        }
