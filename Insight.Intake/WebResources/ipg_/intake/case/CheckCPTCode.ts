...
namespace Intake.Case {
  ...
  */
  function CheckCPTCode(executionContext) {
    ...
    if (CPTCode) {
      Xrm.WebApi.retrieveRecord("ipg_cptcode", CPTCode[0].id, "?$select=ipg_name,ipg_supportedcpt").then(
        function success(result) { 
          ...
          else formContext.getControl(attribute.getName()).clearNotification(fieldName);
        }
      );
    }
    else
  ...
  export function CPTCodeOnChange(executionContext) {
